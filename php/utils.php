...
* Parameters:
*  $response: HTML code (response from getResponse() call)
* Return value:
...
* Parameters:
*  $doc   : DOMDocument object (from getDocument() call)
*  $tags  : Array of tags to search for (ex: ["a","span"])
*  $values: Array of alias => value to extact. 
...
* Parameters:
*  $items: A 2D array of items (from the getItems() call)
*  $cols : An array of column names to extract from $items (ex: ["content","rate"])
*  $cfg  : A set of filters in "column"=>"value" format (ex: ["classname"=>"client-data"]) 
...
* Parameters:
*  $data   : Array of rows to be printed
*  $header : Array of column names. If none is specified, it uses the column names
*/
...
*    - tags     : array of tags to process
*    - elements : array of alias=>elements to be processed
*    - fields	: array of column names to be returned
*    - filters  : array of content filters alias=>valuue
*/
